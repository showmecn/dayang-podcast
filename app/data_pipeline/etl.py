...
def _is_within_years(episode: dict[str, Any], years: int = 3) -> bool:
    """Check if episode was published within N years (for initial seed ingest)."""
    pub = episode.get("published_at")
    if not pub:
        return True  # conservative: include if we don't know the date
    now = datetime.now(timezone.utc)
    delta = now - pub
    return delta.days <= years * 365
...
async def ingest_feed(
    ...
    if incremental:
        ...
        episodes = [ep for ep in episodes if _is_within_years(ep, years=3)]
