...
# Weak/phrasing to avoid in reasons — served as safety net if LLM ignores the prompt
_WEAK_PHRASES = ["间接涉及", "可能相关", "可能有关", "一定程度上涉及", "似乎", "间接"]
_DEFAULT_REASON_TEMPLATE = "This episode directly addresses your query about '{topic}'"


def _sanitize_reason(reason: str, topic: str) -> str:
    """Replace weak or hedging language with a direct statement."""
    if not reason or not reason.strip():
        return _DEFAULT_REASON_TEMPLATE.format(topic=topic)
...
- Be conservative with scores: assign 88+ only to episodes that DIRECTLY address the query
- DO NOT use weak or hedging phrases: avoid "间接涉及" (indirectly involves), "可能相关" (possibly related), "一定程度上涉及" (touches on), "似乎" (seems), "可能" (maybe) — be explicit and direct in your reasoning
...
async def rerank(
    ...
    # Early exit if no API key: use similarity-based scoring
    if not settings.deepseek_api_key or settings.deepseek_api_key.startswith("sk-you"):
        result = []
        for i, c in enumerate(candidates[:top_k]):
            sim = c.get("similarity", 0.0)
            # Map cosine similarity (0.3-0.6 range from all-MiniLM-L6-v2) to 50-99
            # Factor 250 maps 0.35→87 (nearly threshold), 0.4→99 (above threshold)
            score = max(50, min(99, int(sim * 250)))
            result.append({
                **c,
                "reason": _sanitize_reason(f"Matches your query about '{topic}'", topic),
                "relevance_score": score,
            })
        return result

    if len(candidates) <= top_k:
        # No need to re-rank — assign scores based on vector similarity
        result = []
        for i, c in enumerate(candidates):
            sim = c.get("similarity", 0.0)
            # Map cosine similarity (0.3-0.6 range) to 50-99
            # Factor 250 maps 0.35→87 (nearly threshold), 0.4→99 (above threshold)
            score = max(50, min(99, int(sim * 250)))  # map 0-1 to 50-99
            result.append({
                **c,
                "reason": _sanitize_reason(f"Matches your query about '{topic}'", topic),
                "relevance_score": score,
    ...
    for r in sorted(rankings, key=lambda x: x.get("rank", 999)):
        ...
        if ep_id in candidate_map:
            ...
            result.append({
                **base,
                "reason": _sanitize_reason(r.get("reason", ""), topic),
